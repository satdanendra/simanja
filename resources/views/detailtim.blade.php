<x-app-layout>
                ...
                <div class="p-6">
                    ...
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-300">
                                <tr>
                                    <th scope="col" class="px-6 py-4">RK Tim</th>
                            ...
                            <tbody>
                                ...
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
                                    <td class="px-6 py-4">{{ $proyek->rkTim->masterRkTim->rk_tim_kode }}</td>
                                    <td class="px-6 py-4">{{ $proyek->masterProyek->proyek_kode }}</td>
                                    ...
                                    <td class="px-6 py-4">{{ $proyek->masterProyek->proyek_lapangan }}</td>
                                    <td class="px-6 py-4">
                                        @php
                                        $picUser = null;
                                        if ($proyek->pic_id) {
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <button
                                            ...
                                            data-masterproyek-id="{{ $proyek->master_proyek_id }}"
                                            data-pic-id="{{ $proyek->pic_id }}"
                    ...
                    <div class="flex flex-col items-center justify-center rounded-lg border border-gray-200 bg-gray-50 dark:bg-gray-700 dark:border-gray-600 p-12">
                        ...
                        <p class="text-center text-gray-500 dark:text-gray-400 mb-6 max-w-md">Tim ini belum memiliki proyek. Tambahkan proyek untuk mulai mengelola pekerjaan.</p>
                    ...
                    <form action="{{ route('tim.simpan_proyek', $tim->id) }}" method="POST">
                            ...
                            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-300 sticky top-0">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-6 py-3">RK Tim</th>
                                ...
                                <tbody>
                                    @if(isset($availableMasterProyeks) && count($availableMasterProyeks) > 0)
                                    @foreach($availableMasterProyeks as $masterProyek)
                                    <tr class="proyek-row bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors duration-150" data-kode="{{ strtolower($masterProyek->proyek_kode) }}" data-urai="{{ strtolower($masterProyek->proyek_urai) }}">
                                        <td class="w-4 p-4">
                                            <div class="flex items-center">
                                                <input id="checkbox-proyek-{{ $masterProyek->id }}" type="checkbox" name="proyek_ids[]" value="{{ $masterProyek->id }}" class="proyek-checkbox w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                                                <label for="checkbox-proyek-{{ $masterProyek->id }}" class="sr-only">checkbox</label>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4">{{ $masterProyek->rkTim->rk_tim_kode }}</td>
                                        <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">{{ $masterProyek->proyek_kode }}</td>
                                        <td class="px-6 py-4">{{ $masterProyek->proyek_urai }}</td>
                                        <td class="px-6 py-4">{{ $masterProyek->iku_kode }} - {{ $masterProyek->iku_urai }}</td>
                                        <td class="px-6 py-4">
                                            <select name="pic_ids[{{ $masterProyek->id }}]" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white">
                        ...
                        <div id="new-proyek-form" class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg mb-6 hidden">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label for="new_rk_tim_id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">RK Tim</label>
                                    <select name="new_rk_tim_id" id="new_rk_tim_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" required>
                                        <option value="">-- Pilih RK Tim --</option>
                                        @foreach($rkTims as $rkTim)
                                        <option value="{{ $rkTim->id }}">{{ $rkTim->masterRkTim->rk_tim_kode }} - {{ $rkTim->masterRkTim->rk_tim_urai }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                ...
                                <div>
                                    <label for="new_iku_id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">IKU Terkait</label>
                                    <select name="new_iku_id" id="new_iku_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white">
                                        ...
                                        @foreach($ikus as $iku)
                                        <option value="{{ $iku->id }}">{{ $iku->iku_kode }} - {{ $iku->iku_urai }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                ...
                                <div>
                                    <label for="new_pic_id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">PIC Proyek</label>
                                    <select name="new_pic_id" id="new_pic_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white">
                                        <option value="">-- Pilih PIC --</option>
                                        @foreach($tim->users as $user)
                                        <option value="{{ $user->id }}">{{ $user->name }}</option>
                                        @endforeach
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Tambahan JavaScript untuk fitur proyek

            // Checkbox handlers untuk proyek
            ...
            if (editProyekButtons.length > 0 && editProyekForm) {
                editProyekButtons.forEach(button => {
                    button.addEventListener('click', function() {
                        ...
                        const rkTimId = this.getAttribute('data-rktim-id');
                        const masterProyekId = this.getAttribute('data-masterproyek-id');
                        const picId = this.getAttribute('data-pic-id');

                        // Fetch data proyek dari server
                        fetch(`/tim/{{ $tim->id }}/proyek/${proyekId}/edit`)
                            .then(response => response.json())
                            .then(data => {
                                // Set form action
                                editProyekForm.action = `/tim/{{ $tim->id }}/proyek/${proyekId}`;

                                // Populate form fields
                                document.getElementById('edit_rk_tim_id').value = rkTimId;
                                document.getElementById('edit_proyek_kode').value = data.masterProyek.proyek_kode;
                                document.getElementById('edit_proyek_urai').value = data.masterProyek.proyek_urai;

                                // Handle IKU selection
                                const ikuSelect = document.getElementById('edit_iku_id');
                                if (ikuSelect) {
                                    const ikuKode = data.masterProyek.iku_kode;
                                    const options = Array.from(ikuSelect.options);
                                    const optionToSelect = options.find(option => option.text.startsWith(ikuKode));
                                    if (optionToSelect) {
                                        ikuSelect.value = optionToSelect.value;
                                    } else {
                                        ikuSelect.value = "";
                                    }
                                }

                                document.getElementById('edit_rk_anggota').value = data.masterProyek.rk_anggota;
                                document.getElementById('edit_proyek_lapangan').value = data.masterProyek.proyek_lapangan;
