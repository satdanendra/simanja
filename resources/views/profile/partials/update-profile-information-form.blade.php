<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="nip_lama" name="nip_lama" type="text" class="mt-1 block w-full" :value="old('nip_lama', $user->nip_lama)" required autofocus autocomplete="nip_lama" disabled/>
            <x-input-error class="mt-2" :messages="$errors->get('nip_lama')" />
        </div>

        <div>
            <x-input-label for="nip_baru" :value="__('NIP Baru')" />
            <x-text-input id="nip_baru" name="nip_baru" type="text" class="mt-1 block w-full" :value="old('nip_baru', $user->nip_baru)" required autofocus autocomplete="nip_baru" disabled/>
            <x-input-error class="mt-2" :messages="$errors->get('nip_baru')" />
        </div>

        <div>
            <x-input-label for="role" :value="__('Jabatan')" />
            <x-text-input id="role" name="role" type="text" class="mt-1 block w-full" :value="old('role', \App\Enums\Role::from($user->role->value)->name)" required autofocus autocomplete="role" disabled/>
